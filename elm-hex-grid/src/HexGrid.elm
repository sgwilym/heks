...
pixelToHex : Layout -> ( Float, Float ) -> Point
pixelToHex layout ( px, py ) =
    let
        ( _, _, _, _, b0, b1, b2, b3, _ ) =
            getOrientationParams layout.orientation

        ( ptx, pty ) =
            ( (px - layout.originX) / layout.screenX
            , (py - layout.originY) / layout.screenY
            )
